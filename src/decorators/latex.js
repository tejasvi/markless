...
const getTexDecoration = memoize((texString, display) => {
    ...
    return vscode.window.createTextEditorDecorationType({
        ...
        before: {
            ...
            width: svg.width,
            textDecoration: `none;${state.darkMode? " filter: invert(1)" : ""}`,
        },
    })
});
    ...
    function decorateTex () {
        ...
        while ((match = regEx.exec(state.text))) {
            ...
            decorationRanges.get(getTexDecoration(match[2], match[1].length == 2)).push(posToRange(match.index + 1, match.index + match[0].length - 1));
